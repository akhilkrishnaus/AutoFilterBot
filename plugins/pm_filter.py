...
async def next_page(bot, query):
    ...
    if ENABLE_SHORTLINK and settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f""🎭[MN™]☞[{get_size(file.file_size)}] ➵ {file.file_name}",", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f""🎭[MN™]☞[{get_size(file.file_size)}] ➵ {file.file_name}",", callback_data=f'files#{file.file_id}'
                ),
            ]
            for file in files
        ]
    ...
    if files:
        ...
        if ENABLE_SHORTLINK and settings['button']:
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f""🎭[MN™]☞[{get_size(file.file_size)}] ➵ {file.file_name}",", callback_data=f'{pre}#{file.file_id}'
                    ),
                ]
                for file in files
            ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if ENABLE_SHORTLINK and settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f""🎭[MN™]☞[{get_size(file.file_size)}] ➵ {file.file_name}",", url=await get_shortlink(message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
    ...
    if imdb:
        ...
        cap = f"<b>📁 𝐇𝐞𝐫𝐞 𝐢𝐬 𝐖𝐡𝐚𝐭 𝐈 𝐅𝐨𝐮𝐧𝐝 𝐈𝐧 𝐌𝐲 𝐃𝐚𝐭𝐚𝐛𝐚𝐬𝐞 𝐅𝐨𝐫 𝐘𝐨𝐮𝐫 𝐐𝐮𝐞𝐫𝐲 👇</b>"
